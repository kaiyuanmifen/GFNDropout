...
if args.Data=="SVHN":
	...
	validloader = torch.utils.data.DataLoader(validset, batch_size=batch_size*2, shuffle=False)
		...
		for epoch in range(self.max_epoch):
			...
			for idx_train, data in enumerate(trainloader, 0):
				...
				if  "GFNFM" in self.model_type:
					...
					if args.RewardType==0:
						...
						x_valid_augmented = x_valid_augmented_all[0].to(device)
						'''
						...
						#G_metric = self.model._gfn_step(x_mask=x_valid, y_mask=y_valid ,x_reward=x_valid_augmented, y_reward=y_valid)

						#G_metric['tb_loss'] = 0.0324
						end_time_reward_2 = time.time()
						reward_2_time.append(end_time_reward_2-start_time_reward_2)
			...
			if epoch%EVAL_STEP==0:
				...
				with torch.no_grad():
					...
					OOD_batch_accs = []
					OOD_batch_error = []
					total_test_iteration = time.time()
					for idx_test,test_data in enumerate(testloader):
						...
						print(f"Time for augment test: {time.time() - augment_start_time}")	
						
						print(f"One test iteration: {time.time()-start_time_test}")
					OOD_accs.append(np.mean(OOD_batch_accs))
					OOD_testerrors.append(np.mean(OOD_batch_error))
					print(f"Time for total test iteration: {time.time() - total_test_iteration}")
					print("-"*80)
