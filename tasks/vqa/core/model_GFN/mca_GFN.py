            ...
            if "topdown"==self.mask:
                if layer_idx==0:
                    if (EPSILON<self.random_chance) and (self.training):
                        qz_mask_l=self.rand_mask_generator(torch.zeros(batch_size,512).to(device)).to(device)
                    else:
                        qz_mask_l=self.q_z_mask_generators[layer_idx](torch.zeros(batch_size,512),temperature)#784 is an arbitary number here      
                    
                    qz_p_l=self.q_z_mask_generators[layer_idx].prob(torch.zeros(batch_size,512),qz_mask_l)
                else:
