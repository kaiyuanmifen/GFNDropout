...
class SA_GFN(nn.Module):
    def __init__(self, __C):
        super(SA_GFN, self).__init__()
    ...
    def forward(self, x,x_mask,ans):

        x,actual_masks,masks_qz,masks_conditional,LogPF_qz,LogPB_qz,LogPF_BNN,LogPB_BNN,LogPF_qzxy,LogPB_qzxy,Log_pzx,Log_pz=self.GFN_forward(x, x_mask,ans)
    ...
    def GFN_forward(self,x, x_mask,ans):
        ...
        if self.training:#use tempered version of the policy q(z) or q(z|x,y) during training
            ...
            if "topdown"==self.mask:
                if layer_idx==0:
                    if (EPSILON<self.random_chance) and (self.training):
                        qz_mask_l=self.rand_mask_generator(torch.zeros(batch_size,x.shape[1],512).to(self.device)).to(self.device)
                    else:
                        qz_mask_l=self.q_z_mask_generators[layer_idx](torch.zeros(batch_size,x.shape[1],512).to(self.device),temperature)#784 is an arbitary number here      
                    
                    qz_p_l=self.q_z_mask_generators[layer_idx].prob(torch.zeros(batch_size,x.shape[1],512).to(self.device),qz_mask_l)
                else:
                    ##concatenate all previous masks
                    previous_mask=[]
                    for j in range(layer_idx):
                        previous_mask.append(masks_qz[j][-1])
                    previous_mask=torch.cat(previous_mask,2)

                    if (EPSILON<self.random_chance) and (self.training):
                        qz_mask_l=self.rand_mask_generator(previous_mask).to(self.device)
                    else:
                ...
                LogPF_qz+=(qz_mask_l*torch.log(qz_p_l)+(1-qz_mask_l)*torch.log(1-qz_p_l)).sum(2).sum(1)
        ...
        if self.BNN:
            ...
            for layer_idx in range(3):
                EPSILON=random.uniform(0,1)
                if ("bottomup" in self.mask):

                    if self.training:
                        #during training use q(z|x,y;phi) to sample mask
                        #print("using q(z|x,y;phi)")#make sure the right function is used
                        if layer_idx==0:
                            input_qzxy=torch.cat([ans.unsqueeze(1).repeat(1,x.shape[1],1)]+[x.clone().detach()],2)
                            if EPSILON>=self.random_chance:    
                                m_conditional_l= self.q_zxy_mask_generators[layer_idx](input_qzxy,temperature)#generate mask based on activation from previous layer, detach from BNN training
                            else:
                                m=self.rand_mask_generator(torch.zeros(input_qzxy.shape[0],input_qzxy.shape[1])).to(self.device)
                                m_conditional_l=m
                            qzxy_p_l=self.q_zxy_mask_generators[layer_idx].prob(input_qzxy,m_conditional_l)
                            ...
                            if EPSILON>=self.random_chance:
                                m_conditional_l= self.q_zxy_mask_generators[layer_idx](input_qzxy,temperature)
                            else:
                                m=self.rand_mask_generator(torch.zeros(input_qzxy.shape[0],input_qzxy.shape[1])).to(self.device)
                        ...
                        #during inference use p(z|x;xi) to sample mask
                            #print("using p(z|x;xi)")#make sure the right function is used
                            if layer_idx==0:
                ...
                if self.mask=="random":# completely random mask used
                    EPSILON=random.uniform(0,1)
                    ...
                    if self.training:
                        ...
                        if layer_idx==0:
                            ...
                            if EPSILON>=self.random_chance:    
                                ...
                                m=self.rand_mask_generator(torch.zeros(input_qzxy.shape[0],input_qzxy.shape[1])).to(device)
                                m_conditional_l=m
                            qzxy_p_l=self.q_zxy_mask_generators[layer_idx].prob(input_qzxy,m_conditional_l)
...
class MCA_ED_GFN(nn.Module):
    def __init__(self, __C):
        ...
        self.enc_list = nn.ModuleList([SA(__C) for _ in range(__C.LAYER)])#if not selected to be used, then this is a place hodler not in use
        self.dec_list = nn.ModuleList([SGA(__C) for _ in range(__C.LAYER)])#if not selected to be used, then this is a place hodler not in use
        
        self.enc_list_GFN = nn.ModuleList([SA_GFN(__C) for _ in range(__C.LAYER)])
        self.dec_list_GFN = nn.ModuleList([SGA_GFN(__C) for _ in range(__C.LAYER)])
    ...
    def forward(self, x, y, x_mask, y_mask,ans=None):
        ...
        # Get hidden vector, these original masks are used to deal with padding
        ...
        for enc in self.enc_list_GFN:
            x,actual_masks,masks_qz,masks_conditional,LogPF_qz,LogPB_qz,LogPF_BNN,LogPB_BNN,LogPF_qzxy,LogPB_qzxy,Log_pzx,Log_pz = enc(x,x_mask,ans)
        ...
        #decoding visual information plus cross attention to textual information
