...
class VQAEval:
    ...
    def evaluate(self, quesIds=None):
        ...
        for quesId in quesIds:
            ...
            if ansType not in accAnsType:
                accAnsType[ansType] = []
                p_value_listAnsType[ansType] = []
            accAnsType[ansType].append(avgGTAcc)
            p_value_listAnsType[ansType].append(p_value)
        ...
        self.setUncertainty(accQA, accAnsType, p_value_list, p_value_listAnsType)
        print ("Done computing accuracy")
        return {'accQA': accQA, 'accAnsType': accAnsType,
                'p_value_list': p_value_list, 'p_value_listAnsType': p_value_listAnsType}
    ...
    def setUncertainty(self, accQA, accAnsType, p_value_list, p_value_listAnsType):
        accQA = np.array(accQA)

        p_value_list = np.array(p_value_list)
        uc_1 = (p_value_list > 0.01)
        pavpu_1 = np.mean(uc_1 * (1 - accQA) + (1 - uc_1) * accQA)

        uc_2 = (p_value_list > 0.05)
        pavpu_2 = np.mean(uc_2 * (1 - accQA) + (1 - uc_2) * accQA)

        uc_3 = (p_value_list > 0.1)
        pavpu_3 = np.mean(uc_3 * (1 - accQA) + (1 - uc_3) * accQA)

        self.pavpu = [pavpu_1, pavpu_2, pavpu_3]

        self.uncertainty['overall'] = self.pavpu_fun(accQA, np.array(p_value_list))
        self.uncertainty['perAnswerType'] = {ansType: self.pavpu_fun(
            accAnsType[ansType], np.array(p_value_listAnsType[ansType])) for ansType in accAnsType}

    def pavpu_fun(self, acc, p_value):
        uc_1 = (p_value > 0.01)
        acc = np.array(acc)
        pavpu_1 = np.mean(uc_1 * (1 - acc) + (1 - uc_1) * acc) * 100

        uc_2 = (p_value > 0.05)
        pavpu_2 = np.mean(uc_2 * (1 - acc) + (1 - uc_2) * acc) * 100

        uc_3 = (p_value > 0.1)
        pavpu_3 = np.mean(uc_3 * (1 - acc) + (1 - uc_3) * acc) * 100
