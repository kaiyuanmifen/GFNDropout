...
head(data)
data_old=read.csv("../results/cifar10c_testresult_old.csv",header = TRUE)
#data=data[,2:ncol(data)]



names(data_old)=c("load_file","dataset","corruption_name","corruption_severity",
              "Method","BNN","use_pretrained","Tune_last_layer_only",
              "augment_test","seed","val_loss","val_accuracy", "base_aic0",
              "base_aic1","base_aic2", "up0",
              "up1","up2", "elbo", "ece")

data_old=data_old[!data_old$Method%in%c("topdown","bottomup"),]
head(data_old)
#old data to include basedlin 
...
for (corruption_severity in unique(data$corruption_severity)){
  ...
  for (Method in unique(data$Method)){
    ...
    Vec=data.frame(corruption_name="all",corruption_severity=corruption_severity,
