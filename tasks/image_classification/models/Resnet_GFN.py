...
class ResNet_GFN(nn.Module):
	...
	def forward(self, x, y,mask):
		...
		if self.training:#be very careful it should be self.training instead of self.opt.GFN_train
	...
	def GFN_forward(self, x,y,mask="none"):
		...
		LogZ_unconditional=self.LogZ_unconditional#top down mask has partition function indepdent of input x
			...
			if "topdown"==mask:
				...
				if layer_idx==0:
					...
					if (EPSILON<self.random_chance) and (self.training):
						qz_mask_l=self.rand_mask_generator(torch.zeros(batch_size,self.maskgenerator_input_shapes[0][0]).to(device)).to(device)

					else:
						qz_mask_l=self.q_z_mask_generators[layer_idx](torch.zeros(batch_size,784).to(device),temperature)#784 is an arbitary number here      
						
										
					qz_p_l=self.q_z_mask_generators[layer_idx].prob(torch.zeros(batch_size,784).to(device),qz_mask_l)
				else:
					...
					if (EPSILON<self.random_chance) and (self.training):
						...
						qz_mask_l=self.q_z_mask_generators[layer_idx](previous_mask,temperature).to(device)	
											
					qz_p_l=self.q_z_mask_generators[layer_idx].prob(previous_mask.to(device),qz_mask_l.to(device))      
							...
							if self.training:
								...
								if layer_idx==0:
									if EPSILON>=self.random_chance:
										m_conditional_l= self.q_zxy_mask_generators[layer_idx](torch.zeros(batch_size,out.shape[1]).to(device),out.reshape(batch_size,-1).clone().detach(),y.float().clone().detach(),temperature)#generate mask based on activation from previous layer, detach from BNN training
									else:
										m=self.rand_mask_generator(torch.zeros(out.shape[0],out.shape[1])).to(device)
										m_conditional_l=m
									
									qzxy_p_l= self.q_zxy_mask_generators[layer_idx].prob(torch.zeros(batch_size,out.shape[1]).to(device),out.reshape(batch_size,-1).clone().detach(),y.float().clone().detach(),m_conditional_l)
									...
									if EPSILON>=self.random_chance:
										m_conditional_l = self.q_zxy_mask_generators[layer_idx](previous_actual_mask,out.reshape(batch_size,-1).clone().detach(),y.float().clone().detach(),temperature)

									else:# during training ,of a centain chance a random policy will be used to explore the space
								
										m=self.rand_mask_generator(torch.zeros(out.shape[0],out.shape[1])).to(device)
										m_conditional_l=m
									qzxy_p_l= self.q_zxy_mask_generators[layer_idx].prob(previous_actual_mask,out.reshape(batch_size,-1).clone().detach(),y.float().clone().detach(),m_conditional_l)
								...
								#during inference use p(z|x;xi) to sample mask
									...
									if layer_idx==0:
										m_conditional_l= self.p_zx_mask_generators[layer_idx](out.clone().detach().reshape(out.shape[0],-1))
