        ...
        if self.opt.dptype:
            if self.opt.ctype == "Gaussian":
                #def sample_z(self, batch_size, input):
                    ...
                    self.u = self.floatTensor(batch_size, self.in_features).normal_(0, 1).to(self.device)
                    u = self.u
                ...
                #def sample_z(self, batch_size, input):
                    ...
                    # output: batch, 1, weight, height
                    # self.z_phi = self.some_func(input)
                    self.z_phi = self.contextual_dropoutdense(input).to(self.device)
                    if self.opt.hardsigmoid:
                        pi = F.hardtanh(self.opt.k * self.z_phi / 7. + .5, 0, 1).to(self.device)
                    else:
                        pi = torch.sigmoid(self.opt.k * self.z_phi).to(self.device)
                    # updating
                    ...
                    if self.forward_mode:
                        z = self.floatTensor(batch_size, self.in_features).zero_().to(self.device)
                        if self.training:
                            if self.opt.add_pi:
                            ...
                            if self.local_rep:
                                self.u = self.floatTensor(self.in_features).uniform_(0, 1).expand(batch_size,
                                                                                                  self.in_features).to(self.device)
                            else:
                                self.u = self.floatTensor(batch_size, self.in_features).uniform_(0, 1).to(self.device)
                        ...
                        z = self.floatTensor(self.u.size()).zero_().to(self.device)
                        z[self.u > pi2.expand(batch_size, self.in_features)] = 1
                        if self.opt.finetune:
                            z[self.test_z[0:1, :].repeat(batch_size, 1) == 0] = 0
                        z_true = self.floatTensor(self.u.size()).zero_().to(self.device)
                        z_true[self.u < pi] = 1
                        if self.opt.use_t_in_training:
            ...
            #def sample_z(self, batch_size):
                ...
                if self.forward_mode:
                    ...
                    if self.training:
                        if self.local_rep:
                            self.u = self.floatTensor(self.in_features).uniform_(0, 1).expand(batch_size, self.in_features).to(self.device)
                        else:
                            self.u = self.floatTensor(batch_size, self.in_features).uniform_(0, 1).to(self.device)
                        if self.opt.gumbelconcrete:
                            ...
                            z = (torch.log(pi + eps) - torch.log(1 - pi + eps) + torch.log(u + eps) - torch.log(
                                1 - u + eps))
                            z = torch.sigmoid(z / temp).to(self.device)
                        else:
                        ...
                        if self.opt.concretedp:
                            pi = torch.sigmoid(self.opt.k * self.z_phi).to(self.device)
                            self.post_nll_true = - (z * torch.log(pi + epsilon) + (1 - z) * torch.log(1 - pi + epsilon))
                            self.post_nll_true = self.post_nll_true.mean(1).to(self.device)

                            prior_pi = torch.sigmoid(self.opt.k * self.eta.unsqueeze(0)).to(self.device)
                            self.prior_nll_true = - (
                                        z * torch.log(prior_pi + epsilon) + (1 - z) * torch.log(1 - prior_pi + epsilon))
                            self.prior_nll_true = self.prior_nll_true.mean(1)
