...
class ArmConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True,
        ...
        self.u = torch.Tensor(self.dim_z).uniform_(0, 1).to(self.device);
        ...
        if self.opt.dptype:
            if self.opt.dpcha:
                if self.opt.ctype == "Gaussian":
                        ...
                        self.u = self.floatTensor(batch_size, self.out_channels, 1, 1).normal_(0, 1).to(self.device)
                        u = self.u
                        ...
                        # self.z_phi = self.some_func(input_)
                        # if self.layers  = self.layers [i]
                        self.z_phi = self.contextual_dropout(input_).to(device)
                        if self.opt.hardsigmoid:
                            pi = F.hardtanh(self.opt.k * self.z_phi / 7. + .5, 0, 1).to(device)
                        else:
                            pi = torch.sigmoid(self.opt.k * self.z_phi).to(device)
                        # updating
                        ...
                        if self.forward_mode:
                            z = self.floatTensor(batch_size, self.out_channels, 1, 1).zero_().to(device)
                            if self.training:
                                if self.local_rep:
                                    self.u = self.floatTensor(self.out_channels).uniform_(0, 1).expand(batch_size, self.out_channels,1,1).to(self.device)
                                else:
                                    self.u = self.floatTensor(batch_size, self.out_channels, 1, 1).uniform_(0, 1).to(self.device)
                                if self.opt.optim_method:
                                    ...
                                    z.to(device)[self.u.to(device) < pi.to(device)] = 1
                            ...
                            z = self.floatTensor(batch_size, self.out_channels, 1, 1).zero_().to(device)
                            z.to(device)[self.u.to(device) > pi2.to(device)] = 1
                            if self.opt.finetune:
                                z[self.test_z[0:1,:,:,:].repeat(batch_size, 1,1,1) == 0] = 0
                                # self.test_z = z
                            z_true = self.floatTensor(batch_size, self.out_channels, 1, 1).zero_().to(device)
                            z_true.to(device)[self.u.to(device) < pi.to(device)] = 1
                            if self.opt.use_t_in_training:
                                z[pi < self.opt.t] = 0
                            self.sarm_mask = pi.new_ones(batch_size, self.out_channels, 1, 1)
                            self.sarm_mask[z.to(device) == z_true.to(device)] = 0.0
                    ...
                    self.z_phi = self.contextual_dropout(input_)
                    if self.opt.hardsigmoid:
                        pi = F.hardtanh(self.opt.k * self.z_phi / 7. + .5, 0, 1).detach().to(device)
                    else:
                        pi = torch.sigmoid(self.opt.k * self.z_phi).detach().to(device)
                    #updating
                    #print('mean', torch.mean(pi))
                    #print('std', torch.std(pi))
                    self.pi = pi
                    if self.forward_mode:
                        ...
                        if self.training:
                            if self.local_rep:
                                self.u = self.floatTensor(self.dim_z).uniform_(0, 1).expand(batch_size, self.dim_z).to(self.device)
                            else:
                                self.u = self.floatTensor(batch_size, 1, input_.shape[2], input_.shape[3]).uniform_(0, 1).to(self.device)
                            z[self.u < pi] = 1
                            if self.opt.use_t_in_training:
                                z[pi < self.opt.t] = 0
                            self.train_z = z
                ...
                #print('ARMCon', pi.var(), pi.mean())
                if self.forward_mode:
                    z = self.floatTensor(batch_size, self.dim_z).zero_().to(device)
                    if self.training:
                        if self.local_rep:
                            self.u = self.floatTensor(self.dim_z).uniform_(0, 1).expand(batch_size, self.dim_z).to(self.device)
                        else:
                            self.u = self.floatTensor(batch_size, self.dim_z).uniform_(0, 1).to(self.device)
                        ...
                        if self.opt.concretedp:
                            ...
                            pi = torch.sigmoid(self.opt.k * self.z_phi).to(device)
                            self.post_nll_true = - (z * torch.log(pi + epsilon) + (1 - z) * torch.log(1 - pi + epsilon)).to(device)
                            self.post_nll_true = self.post_nll_true.mean(1).squeeze().to(device)
                            # print('test', self.post_nll_true.shape)


                            prior_pi = torch.sigmoid(self.opt.k * self.eta).to(device)
    ...
    def sample_gauss(self, batch_size):
        ...
        self.pi = pi
        if self.local_rep:
            self.u = self.floatTensor(self.dim_z).normal_(1, self.opt.sd).expand(batch_size, self.dim_z).to(self.device)
        else:
            self.u = self.floatTensor(batch_size, self.dim_z).normal_(1, self.opt.sd).to(self.device)
        z = self.u
        self.train_z = z
        self.test_z = z
        return z.view(batch_size, self.dim_z, 1, 1)
