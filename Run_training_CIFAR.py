...
if args.Data=="CIFAR10":
	...
	corrupted_cifar_test = CIFAR_1O_Corrupted(CORRPUTED_FILES_DIR,test_transform)
...
class MLPClassifier:
	def __init__(self, image_size,droprates=0.5,  max_epoch=600, \
				 lr=0.05, momentum=0,model_type="MLP_nodropout",N_units=50):
		# Wrap MLP model
		...
		# ResNet
		if self.model_type=="RESNET_nodropout":
		...
		with torch.no_grad():
			if "GFNFM" in self.model_type:
				...
				for j in range(N_repeats):#repeat N times and sample the distribution
					...
					condition=model.Get_condition(x).detach().clone().to(device)
